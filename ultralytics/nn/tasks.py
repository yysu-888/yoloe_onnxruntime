...
class BaseModel(nn.Module):
    ...
    def forward(self, x,tpe, *args, **kwargs):
        """
        ...
        return self.predict(x,tpe,*args, **kwargs)

    def predict(self, x,tpe,profile=False, visualize=False, augment=False, embed=None,**kwargs):
        """
        ...
        return self._predict_once(x, profile, visualize, embed,**kwargs)
    ...
    def __init__(self, cfg="yolov8n.yaml", ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        if isinstance(m, Detect):  # includes all Detect subclasses like Segment, Pose, OBB, YOLOEDetect, YOLOESegment
            ...
            def _forward(x):
                ...
                return self.forward(x)[0] if isinstance(m, (Segment, YOLOESegment, Pose, OBB)) else self.forward(x,None)
...
class YOLOEModel(DetectionModel):
    ...
    @smart_inference_mode()
    def get_text_pe(self, text, batch=80, cache_clip_model=False,export = False,model_path=None):
        ...
        if (
            ...
            self.clip_model = build_text_model(text_model, device=device,path = model_path)
            
        model = self.clip_model if cache_clip_model else build_text_model(text_model, device=device, path = model_path)
    ...
    def predict(self, x, tpe,profile=False, visualize=False, \
        augment=False, embed=None, vpe=None, return_vpe=False,**kargs):
        """
        Perform a forward pass through the model.
        ...
        for m in self.model:  # except the head part
            ...
            if isinstance(m, C2fAttn):
                ...
                if return_vpe:
                    ...
                    assert(not self.training)
                    return vpe
                cls_pe = self.get_cls_pe(m.get_tpe_v(tpe), vpe).to(device=x[0].device, dtype=x[0].dtype)
                if len(cls_pe) != b:
